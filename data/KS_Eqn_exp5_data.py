...
for train_num in range(n_train):
    ...
    for k in range(2,n_IC,3):
        u = u = A2_vect[square_ind]*np.logical_or(np.logical_or(np.abs(x-c_vect[square_ind])<w_vect[square_ind]/2,np.abs(x+L-c_vect[square_ind]) < w_vect[square_ind]/2),abs(x-L-c_vect[square_ind]) < w_vect[square_ind]/2)
        u_0[k,:] = u - np.mean(u)
        square_ind += 1
