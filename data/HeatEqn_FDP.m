function [U] = HeatEqn_FDP(D,L,x,t,u_0)
...
U(1,:) = u_0;
 ...
 A(1,1) = (1/k)+ D/h^2; A(1,2) = -D/(2*h^2); A(1,n) = -D/(2*h^2);
 A(n,n-1) = -D/(2*h^2); A(n,n) = (1/k)+ D/h^2; A(n,1) = -D/(2*h^2);
...
for i = 1:length(t)-1
    ...
    b(1) = D/(2*h^2)*U(i,n)+(1/k - D/h^2)*U(i,1)+D/(2*h^2)*U(i,2);
    b(n) = D/(2*h^2)*U(i,n-1)+(1/k - D/h^2)*U(i,n)+D/(2*h^2)*U(i,1);
