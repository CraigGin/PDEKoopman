...
# Widths of layers
n_inputs = 128
conv1_filters = 64 # Number of filters in first convolutional layer
n_outputs = 128
...
folder_name = 'CH_exp4e'  # Folder to be created for saved output
...
with tf.name_scope("network"):
    ...
    hidden2 = my_conv_layer(hidden1_reshaped, filters=conv1_filters, kernel_size=1, strides=1, padding="VALID",
                             name="hidden2")
    hidden2_flat = tf.reshape(hidden2, shape=[-1, n_inputs * conv1_filters])
    output = tf.layers.dense(hidden2_flat, n_outputs, name="outputs", activation=None, kernel_initializer=he_init,
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(reg_lam), bias_regularizer=None)
