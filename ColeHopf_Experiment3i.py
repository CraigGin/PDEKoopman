...
folder_name = 'CH_exp3i'  # Folder to be created for saved output
...
with tf.name_scope("network"):
    log_X = tf.log(X+1)
    hidden1 = my_dense_layer(log_X, n_hidden1, name="hidden1")
    hidden2 = my_dense_layer(hidden1, n_hidden2, name="hidden2")
    hidden3 = my_dense_layer(hidden2, n_hidden3, name="hidden3")
    hidden4 = my_dense_layer(hidden3, n_hidden4, name="hidden4")
    log_output = tf.layers.dense(hidden4, n_outputs, name="log_output", activation=None, kernel_initializer=he_init,
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(reg_lam), bias_regularizer=None)
    output = tf.exp(log_output)
