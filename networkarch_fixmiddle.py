...
def encoder(widths, dist_weights, dist_biases, scale, num_shifts_max, first_guess, add_identity):
    ...
    for i in np.arange(len(widths) - 2):
    ...
    large_dim = widths[len(widths)-2]
    DFT = dft(large_dim)
    rDFT = np.real(DFT)
    iDFT = np.imag(DFT)
    combinedDFT = rDFT[0,:]
    for i in xrange(1,large_dim/2):
        combinedDFT = np.vstack((combinedDFT, rDFT[i,:]))
        combinedDFT = np.vstack((combinedDFT, iDFT[i,:]))
    combinedDFT = np.vstack((combinedDFT, rDFT[large_dim/2,:]))
    Reduce = np.hstack((np.eye(widths[len(widths)-1]),np.zeros((widths[len(widths)-1],large_dim-widths[len(widths)-1]))))
    Reduced_DFT = Reduce.dot(combinedDFT)
    Reduced_DFT = Reduced_DFT.T
    no_bias = np.zeros((widths[len(widths)-1], ))

    weights['WE%d' % (len(widths) - 1)] = tf.Variable(Reduced_DFT, name='WE%d' % (len(widths) - 1), trainable=False, dtype=tf.float32)
    biases['bE%d' % (len(widths) - 1)] = tf.Variable(no_bias, name='bE%d' % (len(widths) - 1), trainable=False, dtype=tf.float32)
...
def decoder(widths, dist_weights, dist_biases, scale, name='D', first_guess=0, add_identity=0):
    ...
    # Fix first matrix of decoder to be expanding iDFT
    large_dim = widths[1]
    DFT = dft(large_dim)
    rDFT = np.real(DFT)
    iDFT = np.imag(DFT)
    ...
    for i in xrange(1,large_dim/2):
        ...
        combinedDFT = np.vstack((combinedDFT, iDFT[i,:]))
    combinedDFT = np.vstack((combinedDFT, rDFT[large_dim/2,:]))
    Reduce = np.hstack((np.eye(widths[0]),np.zeros((widths[0],large_dim-widths[0]))))
    Expand = Reduce.T
    inv_DFT = np.linalg.inv(combinedDFT)
    Expand_DFT = inv_DFT.dot(Expand)
    Expand_DFT = Expand_DFT.T
    no_bias = np.zeros((large_dim, ))

    weights['WD1'] = tf.Variable(Expand_DFT, name='WD1', trainable=False, dtype=tf.float32)
    biases['bD1'] = tf.Variable(no_bias, name='bD1', trainable=False, dtype=tf.float32)

    for i in np.arange(1, len(widths) - 1):
        ind = i + 1
    ...
    if add_identity:
        identity_weight = tf.Variable(initial_value=add_identity, name=('alpha%s' % name), dtype=np.float32, trainable=False)
    else:
        identity_weight = 0
    ...
    if not params['autoencoder_only']:
        if not params['fixed_L']:
            ...
            if not params['diag_L']:
                ...
                dt = params['delta_t']
                L = tf.Variable(np.diag(np.exp(-params['mu']*kv*kv*dt)), name='L', trainable=False, dtype=tf.float32)
            else:
                max_freq = np.divide(k,2)
                kv = np.empty((k,))
                if k % 2 == 0:
