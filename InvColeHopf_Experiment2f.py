...
folder_name = 'ICH_exp2f'  # Folder to be created for saved output
...
X_padded = tf.concat([X,tf.slice(X,[0,0],[-1,1])],1)
X_reshaped = tf.reshape(X_padded, shape=[-1, n_inputs+1, 1])
...
with tf.name_scope("network"):
    hidden1 = my_conv_layer(X_reshaped, filters=conv1_filters, kernel_size=2, strides=1, padding="VALID",
                            name="hidden1")
    hidden1_flat = tf.reshape(hidden1, shape=[-1, n_inputs * conv1_filters])
    #hidden2 = tf.layers.max_pooling1d(hidden1, pool_size=2, strides=1, padding="VALID", name="hidden2")
    #hidden2_flat = tf.reshape(hidden2, shape=[-1, n_inputs*conv1_filters])
    hidden2 = my_dense_layer(hidden1_flat, n_hidden2, name="hidden2")
