...
def plot_errors(errors, log_indices):
    ...
    ax[0, 0].scatter(np.arange(num_errors), errors[:, 0], c='r')
    ax[0, 0].scatter(np.arange(num_errors), errors[:, 1], c='b')
    ax[0, 0].set_ylabel('log10(train/test error)')
    ax[0, 1].scatter(np.arange(num_errors), errors[:, 2], c='r')
    ax[0, 1].scatter(np.arange(num_errors), errors[:, 3], c='b')
    ax[0, 1].set_ylabel('log10(reg. train/test error)')
    ax[0, 2].scatter(np.arange(num_errors), errors[:, 4], c='r')
    ax[0, 2].scatter(np.arange(num_errors), errors[:, 5], c='b')
    ax[0, 2].set_ylabel('log10(train/test loss1: autoencoder)')

    ax[1, 0].scatter(np.arange(num_errors), errors[:, 6], c='r')
    ax[1, 0].scatter(np.arange(num_errors), errors[:, 7], c='b')
    ax[1, 0].set_ylabel('log10(train/test loss2: dynamics)')
    ax[1, 1].scatter(np.arange(num_errors), errors[:, 8], c='r')
    ax[1, 1].scatter(np.arange(num_errors), errors[:, 9], c='b')
    ax[1, 1].set_ylabel('log10(train/test loss3: K linear)')

    ax[2, 0].scatter(np.arange(num_errors), errors[:, 10], c='r')
    ax[2, 0].scatter(np.arange(num_errors), errors[:, 11], c='b')
    ax[2, 0].set_ylabel('log10(train/test L_inf)')
    ax[2, 1].scatter(np.arange(num_errors), errors[:, 12], c='r')
    ax[2, 1].scatter(np.arange(num_errors), errors[:, 13], c='b')
    ax[2, 1].set_ylabel('log10(train/test L1)')
    ax[2, 2].scatter(np.arange(num_errors), errors[:, 14], c='r')
    ax[2, 2].scatter(np.arange(num_errors), errors[:, 15], c='b')
    ax[2, 2].set_ylabel('log10(train/test L2)')
    plt.tight_layout()


def load_weights(file_name, num_weights, weight_type='E'):
    W = dict()
    b = dict()
    for j in range(num_weights):
        W1 = np.matrix(np.genfromtxt(file_name.replace("model.pkl", "W%s%d.csv" % (weight_type, j + 1)), delimiter=','))
        b1 = np.matrix(np.genfromtxt(file_name.replace("model.pkl", "b%s%d.csv" % (weight_type, j + 1)), delimiter=','))
        if j > 0:
            if W1.shape[0] != last_size:
                if W1.shape[0] == 1:
                    # need to transpose?
                    if W1.shape[1] == last_size:
                        W1 = np.transpose(W1)
                    else:
                        print "error: sizes %d and %r" % (last_size, W1.shape)
                else:
                    print "error: sizes %d and %r" % (last_size, W1.shape)
        last_size = W1.shape[1]
        W['W%s%d' % (weight_type, j + 1)] = W1
        b['b%s%d' % (weight_type, j + 1)] = b1

    return W, b


def load_weights_koopman(file_name, num_weights, num_weights_omega, num_real, num_complex_pairs):
    d = (num_weights - 1) / 2

    weights, biases = load_weights(file_name, d, 'E')
    W, b = load_weights(file_name, d, 'D')
    weights.update(W)
    biases.update(b)

    for j in np.arange(num_complex_pairs):
        W, b = load_weights(file_name, num_weights_omega, 'OC%d_' % (j + 1))
        weights.update(W)
        biases.update(b)

    for j in np.arange(num_real):
        W, b = load_weights(file_name, num_weights_omega, 'OR%d_' % (j + 1))
        weights.update(W)
        biases.update(b)
...
def stack_data(data, num_shifts, len_time):
    ...
    for j in np.arange(num_shifts + 1):
        for count in np.arange(num_traj):
            indices = np.arange(count * new_len_time, new_len_time + count * new_len_time)
            data_tensor[j, indices, :] = data[count * len_time + j: count * len_time + j + new_len_time, :]
...
def omega_net_apply_one(ycoords, W, b, name, num_weights, act_type):
    if ycoords.shape[1] == 2:
        net_input = np.sum(np.square(ycoords), axis=1)
    else:
        net_input = ycoords

    # want net_input to be [?, 1]
    if len(net_input.shape) == 1:
        net_input = net_input[:, np.newaxis]

    return encoder_apply(net_input, W, b, name, num_weights, act_type)
...
def apply_koopman_net_omegas(x, W, b, delta_t, num_real, num_complex_pairs, num_encoder_weights, num_omega_weights,
                             num_decoder_weights):
    # print x.shape
...
def apply_autoencoder(x, W, b, num_encoder_weights, num_decoder_weights):
    yk = encoder_apply(x, W, b, 'E', num_encoder_weights)
    xk_recon = decoder_apply(yk, W, b, 'D', num_decoder_weights)
...
def eigenfunction_plot(cmap_name, data, outline_x1, outline_y1, outline_x2, outline_y2, X, Y, file_name='',
                       colorbar_flag=True,
                       levels=[], colorbar_ticks=[]):
    plt.figure()
    plt.figure(figsize=(16 / 3 * 2, 16 / 3 * 2))

    im = plt.imshow(data, interpolation='bilinear', origin='lower',
                    cmap=cmap_name, extent=(np.min(X), np.max(X), np.min(Y), np.max(Y)))
    print levels

    if len(levels):
        colors = [(1, 1, 1), (.4, .4, .4), (1, 1, 1)]
        my_map_white_black_white = LinearSegmentedColormap.from_list('white_black_white', colors, N=7)
        plt.contour(data, levels, origin='lower', linewidths=2,
                    extent=(np.min(X), np.max(X), np.min(Y), np.max(Y)), cmap=my_map_white_black_white)
    ...
    if colorbar_flag:
        cbi = plt.colorbar(im, orientation='horizontal', shrink=.7, ticks=colorbar_ticks)
        cbi.ax.set_xticklabels(len(colorbar_ticks) * ['', ])

    if file_name:
        plt.savefig(file_name, dpi=200, transparent=True)


def predict_koopman_net_omegas(x, weights, biases, delta_t, num_steps, num_real, num_complex_pairs, num_encoder_weights,
...
def apply_koopman_net_omegasFull(x_stacked, W, b, delta_t, num_shifts, num_shifts_middle, num_real,
...
def loss_training(params, max_shifts_to_stack, W, b):
    ...
    for j in np.arange(data_train_len):
        ...
        X_stacked = stack_data(X, max_shifts_to_stack, params['len_time'])
        y, g_list = apply_koopman_net_omegasFull(X_stacked, W, b, params['delta_t'], params['num_shifts'],
    ...
    return av_loss1, av_loss2, av_loss3, av_loss_Linf, av_loss, av_regularized_loss


def loss_test(params, max_shifts_to_stack, W, b):
    X = np.loadtxt('%s_test_x.csv' % (params['data_name']), delimiter=',')
    X_stacked = stack_data(X, max_shifts_to_stack, params['len_time'])
    y, g_list = apply_koopman_net_omegasFull(X_stacked, W, b, params['delta_t'], params['num_shifts'],
                                             params['num_shifts_middle'], params['num_real'],
                                             params['num_complex_pairs'],
