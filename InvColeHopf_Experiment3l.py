...
folder_name = 'ICH_exp3l'  # Folder to be created for saved output
...
with tf.name_scope("network"):
    ...
    hidden1 = my_conv_layer(X_reshaped, filters=conv1_filters, kernel_size=5, strides=1, padding="SAME",
                            name="hidden1")
    a = tf.get_variable(name="a", shape=[conv1_filters], dtype=tf.float32, initializer=he_init, 
                         regularizer=tf.contrib.layers.l2_regularizer(reg_lam))
    hidden1_scaled= a*hidden1
    hidden2 = tf.reduce_sum(hidden1_scaled, axis=2, name="hidden2")
    output = tf.layers.dense(hidden2, n_outputs, name="outputs", activation=None, kernel_initializer=he_init,
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(reg_lam), bias_regularizer=None)
