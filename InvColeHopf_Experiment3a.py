...
# Widths of layers
n_inputs = 128
conv1_filters = 16 # Number of filters in first convolutional layer
...
folder_name = 'ICH_exp3a'  # Folder to be created for saved output
data_name = 'ColeHopf_exp3'  # Prefix of data files
...
X_reshaped = tf.reshape(X, shape=[-1, n_inputs, 1])
...
with tf.name_scope("network"):
    hidden1 = my_conv_layer(X_reshaped, filters=conv1_filters, kernel_size=2, strides=1, padding="SAME",
                            name="hidden1")
    hidden2 = tf.layers.max_pooling1d(hidden1, pool_size=2, strides=1, padding="SAME", name="hidden2")
    hidden3 = my_conv_layer(hidden2, filters=conv2_filters, kernel_size=1, strides=1, padding="SAME", 
                            name="hidden3")
    hidden4 = tf.layers.max_pooling1d(hidden3, pool_size=1, strides=1, padding="SAME", name="hidden2")
    hidden4_flat = tf.reshape(hidden4, shape=[-1, n_inputs * conv2_filters])
    output = tf.layers.dense(hidden4_flat, n_outputs, name="outputs", activation=None, kernel_initializer=he_init,
                              kernel_regularizer=tf.contrib.layers.l2_regularizer(reg_lam), bias_regularizer=None)
...
X_train = np.loadtxt(('./data/%s_train1_y.csv' % data_name), delimiter=',', dtype=np.float32)
y_train = np.loadtxt(('./data/%s_train1_x.csv' % data_name), delimiter=',', dtype=np.float32)
for file_num in range(2, data_train_len + 1):
    X_train = np.append(X_train, np.loadtxt(('./data/%s_train%d_y.csv' % (data_name, file_num)), delimiter=',',
                                            dtype=np.float32), 0)
    y_train = np.append(y_train, np.loadtxt(('./data/%s_train%d_x.csv' % (data_name, file_num)), delimiter=',',
                                            dtype=np.float32), 0)

X_valid = np.loadtxt(('./data/%s_val_y.csv' % data_name), delimiter=',', dtype=np.float32)
y_valid = np.loadtxt(('./data/%s_val_x.csv' % data_name), delimiter=',', dtype=np.float32)
