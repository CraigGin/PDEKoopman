...
conv2_filters = 32 # Number of filters in first convolutional layer
conv3_filters = 32 # Number of filters in first convolutional layer
n_outputs = 128
...
folder_name = 'CH_exp4i'  # Folder to be created for saved output
...
with tf.name_scope("network"):
    ...
    hidden1_reshaped = tf.reshape(hidden1, shape=[-1, n_inputs, 1])
    hidden2 = my_conv_layer(hidden1_reshaped, filters=conv1_filters, kernel_size=1, strides=1, padding="SAME",
                             name="hidden2")
    hidden3 = my_conv_layer(hidden2, filters=conv2_filters, kernel_size=1, strides=1, padding="SAME",
                             name="hidden3")
    hidden4 = my_conv_layer(hidden3, filters=conv3_filters, kernel_size=1, strides=1, padding="SAME",
                             name="hidden4")
    a = tf.get_variable(name="a", shape=[conv3_filters], dtype=tf.float32, initializer=he_init, 
                         regularizer=tf.contrib.layers.l2_regularizer(reg_lam))
    hidden4_scaled = a*hidden4
    output = tf.reduce_sum(hidden4_scaled, axis=2, name="outputs")
