...
folder_name = 'ICH_exp3g'  # Folder to be created for saved output
...
with tf.name_scope("network"):
    log_X = tf.log(X)
    dx = 2 * np.pi / n_inputs
    Diff = np.diag(-np.ones(n_inputs-1, dtype=np.float32),k=1) + np.diag(np.ones(n_inputs-1, dtype=np.float32),k=-1)
    Diff[:3,0] = [-3,4,-1]
    Diff[-3:,-1] = [1,-4,3]
    Diff = Diff/(2*dx)
    hidden1 = tf.matmul(log_X,Diff, name="hidden1")
    a = tf.get_variable(name="a", shape=[], dtype=tf.float32, initializer=he_init) 
    output = a*hidden1
