...
conv2_filters = 16 # Number of filters in second convolutional layer
n_outputs = 128
...
folder_name = 'ICH_exp3e'  # Folder to be created for saved output
...
with tf.name_scope("network"):
    log_X = tf.log(X)
    X_reshaped = tf.reshape(log_X, shape=[-1, n_inputs, 1])
    hidden1 = my_conv_layer(X_reshaped, filters=conv1_filters, kernel_size=2, strides=1, padding="SAME",
                            name="hidden1")
    hidden2 = my_conv_layer(hidden1, filters=conv2_filters, kernel_size=1, strides=1, padding="SAME", name="hidden2")
    a = tf.get_variable(name="a", shape=[conv2_filters], dtype=tf.float32, initializer=he_init, 
                         regularizer=tf.contrib.layers.l2_regularizer(reg_lam))
    hidden2_scaled= a*hidden2
    output = tf.reduce_sum(hidden2_scaled, axis=2, name="output")
